...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = RaversideFileSystem.MyFileSystemFactory.class)
public class RaversideFileSystem implements GFileSystem {
	 ...
	 */
	public RaversideFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + RaversideFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<RaversideFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public RaversideFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			RaversideFileSystem fs = new RaversideFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
